...
// The helper requested by the user
const helperCode = 
...
if (!content.includes('findDuplicateImages')) {
    content = content.replace('export type TourismProviderType =', helperCode + '\nexport type TourismProviderType =');
}

// Ensure unique images for MOCK_DATA.hotels
...
// We will do a generic replacement by looking for the item names and replacing their image URLs
hotelImageReplacements.forEach(rep => {
  const regex = new RegExp((name|title):\\s*""[\\s\\S]*?(?:image|images):\\s*(?:\\[\\s*)?"([^"]+)", 'g');
...
console.log('mockData.ts successfully updated with unique paths and duplicate check function!');
